...
static int oled_i2c_fd = -1;

#define OLED_I2C_ADDR 0x3C
// 使用 /dev/i2c-4
#define OLED_I2C_DEV "/dev/i2c-4"

uint8_t OLED_DisplayBuf[8][128];

int OLED_HW_I2C_Init(void)
{
    oled_i2c_fd = open(OLED_I2C_DEV, O_RDWR);
    if(oled_i2c_fd < 0)
    {
        perror("open /dev/i2c-4 failed");
        return -1;
    }
    // 绑定从设备地址
    if(ioctl(oled_i2c_fd, I2C_SLAVE, OLED_I2C_ADDR) < 0)
    {
...
{
    if(oled_i2c_fd < 0) return;
    uint8_t buf[2] = {0x00, Command};
    write(oled_i2c_fd, buf, 2);
}

void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    if(oled_i2c_fd < 0 || Count == 0) return;
    uint8_t tx_buf[129];
    tx_buf[0] = 0x40;  
    memcpy(&tx_buf[1], Data, Count);
    write(oled_i2c_fd, tx_buf, Count + 1);
}
